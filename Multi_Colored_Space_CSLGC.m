...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% LBP CALCULATION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for i=3:m-2
        for j=3:n-2        
            ...
            if (G(i,j-2)-G(i,j+2)>=0); GH7=2^7; else; GH7=0; end
            if (B(i,j-2)-B(i,j+2)>=0); BH7=2^7; else; BH7=0; end
            ...
            if (R(i-2,j)-R(i+2,j)>=0); RH5=2^5; else; RH5=0; end
            if (G(i-2,j)-G(i+2,j)>=0); GH5=2^5; else; GH5=0; end
            if (B(i-2,j)-B(i+2,j)>=0); BH5=2^5; else; BH5=0; end
            if (R(i-1,j)-R(i+1,j)>=0); RH4=2^4; else; RH4=0; end
            if (G(i-1,j)-G(i+1,j)>=0); GH4=2^4; else;GH4=0; end
            if (B(i-1,j)-B(i+1,j)>=0); BH4=2^4; else; BH4=0; end
